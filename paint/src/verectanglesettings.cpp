    ...
    currentRectangle(nullptr)
{
    ...
    connect(ui->comboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),this, &VERectangleSettings::comboBoxIndexChanged);
...
void VERectangleSettings::comboBoxIndexChanged(int index)
{
    switch (index) {
    case 1:
        ui->labelColor_1->setText(("color 1"));
        if(currentRectangle != nullptr){
            setGradient(currentRectangle);
        }
        break;
    default:
        ui->labelColor_1->setText(("background"));
        if(currentRectangle != nullptr){
            currentRectangle->setBrush(QBrush(m_color_1));
        }
        break;
